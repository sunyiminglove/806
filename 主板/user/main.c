...
void zuoxuan(u8 FX) //左旋
{
	...
	{
		Xuan_Zhuang_Ci_Shu--;
		while(PLC_Data[3]!=0xffff&&FX_flag==4)
		{
			osdelay_ms(20);
		}	
...
{	
		...
		Qian_Jin();
		Qi_Dong();	
		while(keynumber!=HmiChuDianTiDiBiao)//一楼门状态（开1/关0）
		{	
			PLC2_Data[22]=10;				//开门信号赋值为10以便调度系统确认门敞开
			osdelay_ms(20);
		}
		...
		while(PLC2_Data[27]!=0)//一楼门状态（开1/关0）
		{
			PLC2_Data[22]=1;//开门信号置1以便电梯门不是常开状态
			PLC2_Data[23]=0;//关门	
			osdelay_ms(20);
...
void AGV_QuSanLou()//AGV去三楼函数
{
		...
		PLC2_Data[23]=0;//关门
		//等待三楼门关到位
		while(PLC2_Data[26]!=0)//三楼门状态（开1/关0）
		{
			PLC2_Data[22]=1;//开门信号置1以便电梯门不是常开状态
			PLC2_Data[23]=0;//关门
			osdelay_ms(20);
...
void UserConfigInit(void)
{
	...
	{
		if(SystemParameter[52]!=2)//1车或者新板子刷1车的系统参数
		{
				#if 1						//为1车的前100个内存中写入初始量
				SystemParameter[0]=5;	//判断是否第一次写入的标志
				SystemParameter[1]=0;
				SystemParameter[2]=0;
				SystemParameter[3]=0;
				SystemParameter[4]=122;//左旋角度
				SystemParameter[5]=120;//右旋角度
				SystemParameter[6]=320;		//初始速度
				SystemParameter[7]=0;     
				SystemParameter[8]=11;    //前进方向Kp
				SystemParameter[9]=300;   //前进方向Ki
				SystemParameter[10]=0;    //前进方向Kd
				SystemParameter[11]=10;   //前进方向的PID控制周期
				SystemParameter[12]=650;  //叉车的舵轮中值
				SystemParameter[13]=2;    //叉臂Kp
				SystemParameter[14]=300;  //叉臂Ki
				SystemParameter[15]=0;    //叉臂Kd
				SystemParameter[16]=0;    
				SystemParameter[17]=0;    
				SystemParameter[18]=250; //低速    
				SystemParameter[19]=320; //中速   
				SystemParameter[20]=600; //高速   
				SystemParameter[21]=0;    
				SystemParameter[22]=0;    
				SystemParameter[23]=0;    
				SystemParameter[24]=7;  //路径号
				SystemParameter[25]=2;  //流程号  
				...
				SystemParameter[59]=12;    //叉臂上升时间
				
				SystemParameter[60]=45;   //叉臂方向的控制系数
				SystemParameter[61]=53;   //叉臂方向的控制系数
				SystemParameter[62]=58;   //叉臂方向的控制系数
				SystemParameter[63]=68;   //叉臂方向的控制系数
				SystemParameter[64]=72;   //叉臂方向的控制系数
				SystemParameter[65]=80;   //叉臂方向的控制系数
				SystemParameter[66]=90;   //叉臂方向的控制系数
				SystemParameter[67]=100;  //叉臂方向的控制系数
				SystemParameter[68]=115;  //叉臂方向的控制系数
				SystemParameter[69]=128;  //叉臂方向的控制系数
				SystemParameter[70]=132;  //叉臂方向的控制系数
				SystemParameter[71]=140;  //叉臂方向的控制系数
				SystemParameter[72]=160;  //叉臂方向的控制系数
				SystemParameter[73]=165;  //叉臂方向的控制系数
				SystemParameter[74]=170;  //叉臂方向的控制系数
				SystemParameter[75]=62;   //叉臂方向的控制系数
				SystemParameter[76]=155;  //叉臂方向的控制系数
				...
				SystemParameter[98]=29;//流程总数
				SystemParameter[99]=47;//路径总数
				#endif
		}
		else											//2车就刷2车的系统参数
		{
				#if 1						//为2车的前100个内存中写入初始量
				SystemParameter[0]=5;	//判断是否第一次写入的标志
				SystemParameter[1]=0;
				SystemParameter[2]=0;
				SystemParameter[3]=0;
				SystemParameter[4]=0;//左旋角度
				SystemParameter[5]=0;//右旋角度
				SystemParameter[6]=320;		//初始速度
				SystemParameter[7]=0;     
				SystemParameter[8]=11;    //前进方向Kp
				SystemParameter[9]=300;   //前进方向Ki
				SystemParameter[10]=0;    //前进方向Kd
				SystemParameter[11]=10;   //前进方向的PID控制周期
				SystemParameter[12]=566;  //叉车的舵轮中值
				SystemParameter[13]=55;   //叉臂Kp
				SystemParameter[14]=5;  	//叉臂Ki
				SystemParameter[15]=250;  //叉臂Kd
				SystemParameter[16]=0;    
				SystemParameter[17]=0;    
				SystemParameter[18]=252; //低速    
				SystemParameter[19]=335; //中速   
				SystemParameter[20]=600; //高速   
				SystemParameter[21]=0;    
				SystemParameter[22]=0;    
				SystemParameter[23]=0;    
				SystemParameter[24]=7;  //路径号
				SystemParameter[25]=27;  //流程号  
				SystemParameter[26]=0;  //任务号  
				SystemParameter[27]=1;  //电梯地标1 
				SystemParameter[28]=3;  //电梯地标3  
				SystemParameter[29]=2;  //出电梯地标
				...
				SystemParameter[57]=0;    
				SystemParameter[58]=0;    
				SystemParameter[59]=10;    //叉臂上升时间
				
				SystemParameter[60]=45;   //叉臂方向的控制系数
				SystemParameter[61]=48;   //叉臂方向的控制系数
				SystemParameter[62]=52;   //叉臂方向的控制系数
				SystemParameter[63]=65;   //叉臂方向的控制系数
				SystemParameter[64]=76;   //叉臂方向的控制系数
				SystemParameter[65]=80;   //叉臂方向的控制系数
				SystemParameter[66]=90;   //叉臂方向的控制系数
				SystemParameter[67]=100;  //叉臂方向的控制系数
				SystemParameter[68]=110;  //叉臂方向的控制系数
				SystemParameter[69]=120;  //叉臂方向的控制系数
				SystemParameter[70]=128;  //叉臂方向的控制系数
				SystemParameter[71]=140;  //叉臂方向的控制系数
				SystemParameter[72]=160;  //叉臂方向的控制系数
				SystemParameter[73]=165;  //叉臂方向的控制系数
				SystemParameter[74]=170;  //叉臂方向的控制系数
				SystemParameter[75]=58;   //叉臂方向的控制系数
				SystemParameter[76]=155;  //叉臂方向的控制系数
				SystemParameter[77]=0;
				SystemParameter[78]=0;
				SystemParameter[79]=0;
				...
				SystemParameter[88]=0;
				SystemParameter[89]=0;
				SystemParameter[98]=28;//流程总数
				SystemParameter[99]=42;//路径总数
				#endif
			...
			if(qj_flag==0)
			{
				...
				switch(Chu_Gui_dir)//当按启动时才判断一次
				{
					...
					case 1://上次出轨是从左边出的
					{
						...
						while((CDH12_num<=9||CDH12_num>=20)&&Chu_Gui_time<200&&QD_flag==1)//没有转到合适的值时或旋转所定时间到之前一直转
						{
...
{	
	...
	{		
		...
		if(CDH12_num!=0)//未出轨时（不居中时）
		{
			...
			if(CDH12_num>=9&&CDH12_num<=20)//中间范围内
			{
				Chu_Gui_dir=0;
			}
			else if(CDH12_num<9)//磁导航太偏左
			{
				Chu_Gui_dir=2;
			}
			else if(CDH12_num>20)//磁导航太偏右
			{
				Chu_Gui_dir=1;
			}
		}
...
//											HmiScreenSetGet = lastscreenNum;
			...
			case 54://搬运到三楼(半自动)
				{
					HmiProcessNum = 28;
					while(HmiScreenSetGet==54)
					{
						PLC_Data[121] = HmiProcessNum-27;
						PLC_Data[122] = PLC_Data[121]*8;
				...
				break;
			case 55://全自动与半自动和电梯口去三楼的方式：电梯口上段
			{
				HmiProcessNum = 34;
				while(HmiScreenSetGet==55)
				{
					if(PLC_Data[123]==1)
					{
						PLC_Data[123]=0;
