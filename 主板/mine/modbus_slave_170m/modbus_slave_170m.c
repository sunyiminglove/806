...
u16 PLC2_Data[64];//PLC数据寄存器

u8 STATIONNUM_170M = 2;//车的站号
...
{
	Uart3_Start_DMA_Tx(sendlength2);
}
...
void Recirve2_02()// 搞定****************************************************
 {
	...
	{
	for(i=0;i<32;i++)
	 PLC2_InPut[i]=(~(P3>>i))&0x01;            //先读出输入口的状态
	// 	for(i=12;i<32;i++)
...
void Recirve2_03()
 {
	...
	// 		 send3_buf[3+i*2]=I2c_Buf[(c)*2];   //返回寄存器值的高字节
	// 		 send3_buf[4+i*2]=I2c_Buf[(startadd+i)*2+1];        //返回寄存器值得低字节
	// 		 
