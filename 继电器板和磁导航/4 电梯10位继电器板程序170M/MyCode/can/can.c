...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
	...
	if(RxMessage.Data[2] == 0x55)//Ö÷¿Ø°å¸´Î»´¦Àí
	{
		Can_Send_Msg(cansend,3);
	}
	JDQ0 = ~((RxMessage.Data[0]^0xff)&0x01);
	JDQ1 = ~(((RxMessage.Data[0]^0xff)>>1)&0x01);
	JDQ2 = ~(((RxMessage.Data[0]^0xff)>>2)&0x01);
	JDQ3 = ~(((RxMessage.Data[0]^0xff)>>3)&0x01);
	JDQ4 = ~(((RxMessage.Data[0]^0xff)>>4)&0x01);
	JDQ5 = ~(((RxMessage.Data[0]^0xff)>>5)&0x01);
	JDQ6 = ~(((RxMessage.Data[0]^0xff)>>6)&0x01);
	JDQ7 = ~(((RxMessage.Data[0]^0xff)>>7)&0x01);
	
	JDQ8 = ~((RxMessage.Data[1]^0xff)&0x01);
	JDQ9 = ~(((RxMessage.Data[1]^0xff)>>1)&0x01);
	JDQ10 = ~(((RxMessage.Data[1]^0xff)>>2)&0x01);
	JDQ11 = ~(((RxMessage.Data[1]^0xff)>>3)&0x01);
}
#endif
...
{	
	...
	TxMessage.ExtId=0x12;			// ÉèÖÃÀ©Õ¹±êÊ¾·û 
...
u8 canref[2]={0xff,0xff},cansend[3]={0xff,0xff,0x55};
void send(void)
{
	u8 temp[2];
	temp[0] = IN0|(IN1<<1)|(IN2<<2)|(IN3<<3)|(IN4<<4)|(IN5<<5)|(IN6<<6)|(IN7<<7);
	temp[1] = IN8|(IN9<<1)|(IN10<<2);
